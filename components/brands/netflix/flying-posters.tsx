...
export interface AutoPlayIntroOptions {
  durationMs: number;
  totalScroll: number;
  onComplete?: () => void;
}
...
interface CanvasParams {
  ...
  cameraFov: number;
  cameraZ: number;
  autoPlayIntro?: AutoPlayIntroOptions;
}
...
uniform float uPolaroidBottom; // thicker bottom border (UV space, e.g. 0.12)

varying vec2 vUv;

void main() {
  float border = 0.03;
  float bottomBorder = uPolaroidBottom;

  if (vUv.x < border || vUv.x > 1.0 - border || vUv.y > 1.0 - border || vUv.y < bottomBorder) {
  ...
  float innerW = 1.0 - 2.0 * border;
  float innerH = 1.0 - border - bottomBorder;
  vec2 innerUv = (vUv - vec2(border, bottomBorder)) / vec2(innerW, innerH);
  vec2 texUv = innerUv * scale + (1.0 - scale) * 0.5;

  gl_FragColor = texture2D(tMap, texUv);
}
`;
...
function AutoBind(self: object, { include, exclude }: AutoBindOptions = {}) {
  ...
  for (const [object, key] of getAllProperties(
    self.constructor.prototype as object,
  )) {
    if (key === "constructor" || !filter(key)) continue;
    const descriptor = Reflect.getOwnPropertyDescriptor(object, key);
    if (descriptor && typeof descriptor.value === "function") {
      (self as Record<string | symbol, unknown>)[key] = (
        (self as Record<string | symbol, unknown>)[key] as (
          ...args: unknown[]
        ) => unknown
      ).bind(self);
    }
...
class Canvas {
  ...
  introProgress: number | null = null;
  introDuration = 0;
  introTotalScroll = 0;
  ...
  constructor({
    ...
    this.onIntroComplete = undefined;

    if (autoPlayIntro) {
      this.introProgress = 0;
      this.introDuration = autoPlayIntro.durationMs / 1000;
      this.introTotalScroll = autoPlayIntro.totalScroll;
      this.onIntroComplete = autoPlayIntro.onComplete;
    }

    AutoBind(this as object);
    this.createRenderer();
    this.createCamera();
    this.createScene();
    this.onResize();
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    if (this.introProgress !== null && this.introProgress < 1) return;
    this.isDown = true;
  ...
  update() {
    ...
    const dt = this.lastTime ? Math.min(now - this.lastTime, 1 / 30) : 1 / 60;
    this.lastTime = now;

    if (this.introProgress !== null && this.introProgress < 1) {
      this.introProgress = Math.min(1, this.introProgress + dt / this.introDuration);
      const easeOut = 1 - Math.pow(1 - this.introProgress, 2);
      this.scroll.target = this.introTotalScroll * easeOut;
